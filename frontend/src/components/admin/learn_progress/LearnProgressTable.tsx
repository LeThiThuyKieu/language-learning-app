...
import type { UserLearnSummary } from "@/services/admin/learnProgressService";
...
export default function LearnProgressTable({
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="bg-gray-50 border-b border-gray-100 text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                                ...
                                <th className="px-5 py-4">Đang học</th>
                                <th className="px-6 py-4 text-right w-32 whitespace-nowrap">Hành động</th>                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-50">
                            {loading ? (
                                ...
                                const pct = user.totalTrees > 0
                                    ? Math.round((user.completedTrees / user.totalTrees) * 100)
                                    : 0;
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-4">
                                            {user.currentLevelName ? (
                                                <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${LEVEL_BADGE[user.currentLevelId ?? 0] ?? "bg-gray-100 text-gray-500"}`}>
                                                    {user.currentLevelName}
                                                </span>
                                            ) : (
                                        ...
                                        <td className="px-4 py-4">
                                            <div className="flex items-center gap-2 min-w-[120px]">
                                                ...
                                                <span className="text-xs font-bold text-gray-600 shrink-0">
                                                    {user.completedTrees}/{user.totalTrees}
                                                </span>
                                            </div>
                                            <p className="text-[10px] text-gray-400 mt-0.5">{pct}% tree hoàn thành</p>
                                        </td>

                                        {/* Current progress */}
                                        <td className="px-8 py-4">
                                            <span className="text-xs font-medium text-gray-600">
                                                {user.currentProgressLabel ?? "—"}
                                            </span>
                                        </td>
