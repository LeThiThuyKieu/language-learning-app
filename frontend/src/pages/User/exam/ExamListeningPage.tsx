    ...
    return (
        <div className="flex items-center justify-between px-5 py-3 bg-white border-b border-gray-200 shadow-sm">
            ...
            <div className="flex items-center gap-2">
                <div
                    className={`flex items-center gap-2 rounded-lg px-5 py-2 text-sm font-extrabold text-white ${urgent ? "bg-red-700" : "bg-red-500"}`}>
...
function PartNavBar({
    ...
    return (
        <div className="flex items-stretch border-t-2 border-gray-300 bg-white">
            {parts.map((part, pIdx) => {
                const answered = part.questions.reduce((sum, q) => {
                    ...
                    if (nums.length > 1) {
                        ...
                        try {
                            parsedAns = answers[q.mongoDocId] ? JSON.parse(answers[q.mongoDocId]) : {};
                        } catch { /* ignore */
                        }
            ...
            <span
                className={`whitespace-nowrap ${isActive ? "font-extrabold text-primary-700" : "font-medium text-gray-500"}`}>
              Part {part.partNumber}
                ...
                {part.questions.flatMap((q, qIdx) => {
                    ...
                    if (nums.length > 1 && answers[q.mongoDocId]) {
                        try {
                            parsedAnswer = JSON.parse(answers[q.mongoDocId]);
                        } catch { /* ignore */
                        }
                    }
                    ...
                    return nums.map((num) => {
                        ...
                        {isBookmarked && num === start && (
                            <svg
                                ...
                                <span onClick={(e) => {
                                    e.stopPropagation();
                                    onGoToQuestion(pIdx, qIdx, nums.length > 1 ? num : undefined);
                                }}
                          ...
                          {num}
                            ...
                            <span
                                className="text-xs text-gray-400 font-medium whitespace-nowrap">{answered} of {total}</span>
                        )}
                    </button>
                );
            })}
            ...
            <div className="w-full">
                ...
                <div className="flex flex-col gap-4">
                    {lines.map((line, lineIdx) => {
                        ...
                        return (
                            <div key={lineIdx}
                                 className="inline-flex items-center flex-wrap gap-x-1 gap-y-1 text-base text-gray-800">
                                                ...
                                                {(() => {
                                                    ...
                                                    if (blankOpts && blankOpts.length > 0) {
                                                        ...
                                                        return (
                                                            <select
                                                                ...
                                                                className="w-full rounded-md border-2 border-blue-300 bg-blue-50 px-2 py-1.5 text-base font-semibold text-gray-800 focus:border-primary-500 focus:bg-white focus:outline-none transition cursor-pointer"
                                                            >
                                                    ...
                                                    return (
                                                        <input
                                                            ...
                                                            onChange={(e) => onBlankChange(currentNum, e.target.value)}
                                                            onFocus={() => onFocusBlank(currentNum)}
                                                            className="w-52 rounded-md border-2 border-blue-300 bg-blue-50 px-3 py-1.5 text-base font-semibold text-gray-800 focus:border-primary-500 focus:bg-white focus:outline-none transition"
                                                        />
                                                    );
                                                })()}
                      </span>
...
function QuestionView({
    ...
    if (question.questionType === "MULTIPLE_CHOICE") {
        ...
        return (
          ...
          <span
              className="flex h-7 w-7 shrink-0 items-center justify-center rounded bg-primary-600 text-white text-xs font-black">
                        ...
                        {(question.options ?? []).map((opt) => (
                            <button key={opt.id} type="button" onClick={() => onAnswer(opt.id)}
                                ...
                                <div className={`overflow-hidden rounded-lg border-4 transition-all bg-white ${
                                    ...
                                    <img src={opt.image_url!} alt={`Option ${opt.id}`}
    ...
    if (question.questionType === "FILL_IN_FORM") {
        ...
        let parsedAnswer: Record<string, string> = {};
        try {
            parsedAnswer = answer ? JSON.parse(answer) : {};
        } catch { /* ignore */
        }
                ...
                <div className="flex gap-10 flex-wrap items-start">
                  ...
                  <span
                    ...
                    {qNum}
                                    ...
                                    <span
                                        className="text-base font-semibold text-gray-800 w-24">{item.label as string}</span>
                                    {/* Drop zone */}
                                    <div
                                    ...
                                    >
                                        {selected ? (
                                            <>
                                                <span
                                                    className="text-sm font-bold text-primary-700 px-3">{selected}</span>
                                                <button
                                                    type="button"
...
export default function ExamListeningPage() {
    ...
    const handleStartAudio = useCallback(() => {
        ...
        // Parse format: {url1,url2,...} hoặc url đơn
        let urls: string[] = [];
        const trimmed = audioUrl.trim();
        if (trimmed.startsWith("{") && trimmed.endsWith("}")) {
            urls = trimmed.slice(1, -1).split(",").map((u) => u.trim()).filter(Boolean);
        } else {
            urls = [trimmed];
        }
        ...
        const playNext = () => {
            ...
            audio.onerror = (e) => {
                console.error(`[Audio] Failed to load (${idx + 1}/${urls.length}):`, url, e);
                idx++;
                playNext(); // Bỏ qua file lỗi, phát tiếp
            };
        ...
        return (
            <div className="flex h-screen items-center justify-center bg-[#e8eef2]">
                <div className="flex flex-col items-center gap-3">
                    <div
                        className="h-10 w-10 rounded-full border-4 border-primary-500 border-t-transparent animate-spin"/>
    ...
    return (
        <div className="flex flex-col h-screen bg-[#e8eef2] overflow-hidden">
            ...
            {parts.length > 0 && (
                <PartNavBar
                    ...
                    onGoToPart={(p) => {
                        setActivePartIdx(p);
                        setActiveQIdx(0);
                        setFocusBlankNum(null);
                    }}
                    onGoToQuestion={(p, q, blankNum) => {
                        setActivePartIdx(p);
                        setActiveQIdx(q);
                        setFocusBlankNum(blankNum ?? null);
                    }}
                            ...
                            <div className="px-6 py-5">
                                ...
                                {unanswered > 0
                                    ? <div
                                        ...
                                        chưa trả lời: {unanswered}</div>
                                    : <div
                                        className="rounded-xl bg-green-50 border border-green-200 px-4 py-3 text-sm font-semibold text-green-700">Bạn
                                        đã trả lời tất cả {totalQ} câu hỏi.</div>
                                }
                            </div>
            ...
            <LessonExitModal
                open={showExitModal} onContinue={() => setShowExitModal(false)} onExit={() => {
                stopAudio();
                navigate(-1);
            }}
            ...
            {showTimeUpModal && (() => {
                ...
                return (
                    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
                        ...
                        <div className="relative w-full max-w-md rounded-2xl bg-white shadow-2xl overflow-hidden">
                            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                                <h2 className="text-lg font-extrabold text-gray-900">Hết giờ Listening!</h2>
                            </div>
                            <div className="px-6 py-5">
                                <p className="text-sm text-gray-600 mb-4">
                                    Thời gian làm bài đã kết thúc. Bài của bạn đã được nộp tự động.
                                </p>
                                {unanswered > 0
