...
import {examService, type ExamPartDto, type ExamQuestionDto, type GradeResponse, type SourceText} from "@/services/examService";
    ...
    return (
        <div className="absolute inset-0 z-30 flex flex-col items-center justify-center bg-[#5a5a5a]/90">
            <div className="mb-6 flex h-24 w-24 items-center justify-center rounded-full border-[3px] border-white">
                <svg className="h-12 w-12 text-white" fill="none" stroke="currentColor" strokeWidth="1.5"
                     viewBox="0 0 24 24">
    ...
    return (
        <div className="flex items-center justify-between px-5 py-3 bg-white border-b border-gray-200 shadow-sm">
            ...
            <div className="flex items-center gap-2">
                <div
                    className={`rounded-lg px-5 py-2 text-sm font-extrabold text-white ${urgent ? "bg-red-700" : "bg-red-500"}`}>
              ...
              Part {part.partNumber}
                ...
                {part.questions.flatMap((q, qIdx) => {
                    ...
                    if (nums.length > 1 && answers[q.mongoDocId]) {
                        try {
                            parsedAns = JSON.parse(answers[q.mongoDocId]);
                        } catch { /* ignore */
                        }
                    }
                        ...
                        {currentNum}
                                ...
                                {blankOpts && blankOpts.length > 0 ? (
                                    <select
                                        ...
                                        className={`w-52 rounded border-2 px-2 py-1 text-sm font-semibold text-gray-800 focus:outline-none transition cursor-pointer ${
                                            isActive
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => handleBlankChange(currentNum, e.target.value)}
                                        onFocus={() => onFocusBlank(currentNum)}
                                        className={`w-52 rounded border-2 px-2 py-1 text-sm font-semibold text-gray-800 focus:outline-none transition ${
                                            isActive
                                                ? "border-primary-500 bg-primary-50"
                                                : "border-blue-300 bg-blue-50 focus:border-primary-500 focus:bg-white"
                                        }`}
...
function QuestionView({
    ...
    const BookmarkBtn = ({docId, bmarked}: { docId: string; bmarked: boolean }) => (
        <button type="button" onClick={() => onToggleBookmarkFor(docId)} className="shrink-0 mr-2 transition-colors">
            <svg
                className={`h-6 w-6 transition-colors ${bmarked ? "fill-primary-500 text-primary-500" : "fill-none text-gray-400 hover:text-gray-600"}`}
